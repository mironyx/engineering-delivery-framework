...
Build a map keyed by **stable IDs** (per ADR-0026):
...
Older requirements docs may not have REQ- anchors (per ADR-0026 scope decision —
historical artefacts in some projects are not retrofitted). For those, fall back
to story-number heuristics; explicitly note the fallback in the report.
