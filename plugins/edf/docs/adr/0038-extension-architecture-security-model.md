...
- **`postMessage` hover path needs validation.** VSCode's markdown preview webview
  supports `postMessage`, but the specific interaction of mouseover on SVG `<a>` elements
  inside Mermaid renderings has not been proven in production. An implementation spike
  should confirm the API works for hover events before Story 2.1 is considered complete.
  If it does not, the fallback is a click-only interaction (no hover tooltip).
