...
case "$LANG" in
    ts) exec bash "$SCRIPT_DIR/typescript/run-typecheck.sh" "$@" ;;
    p)  exec bash "$SCRIPT_DIR/python/run-typecheck.sh" "$@" ;;
    all)
        echo "=== TypeScript typecheck ==="
        bash "$SCRIPT_DIR/typescript/run-typecheck.sh" "$@"; ts_rc=$?
        echo ""
        echo "=== Python typecheck ==="
        bash "$SCRIPT_DIR/python/run-typecheck.sh" "$@"; py_rc=$?
