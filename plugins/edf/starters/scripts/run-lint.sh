...
case "$LANG" in
    ts) exec bash "$SCRIPT_DIR/typescript/run-lint.sh" "$@" ;;
    p)  exec bash "$SCRIPT_DIR/python/run-lint.sh" "$@" ;;
    all)
        echo "=== TypeScript lint ==="
        bash "$SCRIPT_DIR/typescript/run-lint.sh" "$@"; ts_rc=$?
        echo ""
        echo "=== Python lint ==="
        bash "$SCRIPT_DIR/python/run-lint.sh" "$@"; py_rc=$?
