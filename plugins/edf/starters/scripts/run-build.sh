...
case "$LANG" in
    ts) exec bash "$SCRIPT_DIR/typescript/run-build.sh" "$@" ;;
    p)  exec bash "$SCRIPT_DIR/python/run-build.sh" "$@" ;;
    all)
        echo "=== TypeScript build ==="
        bash "$SCRIPT_DIR/typescript/run-build.sh" "$@"; ts_rc=$?
        echo ""
        echo "=== Python build ==="
        bash "$SCRIPT_DIR/python/run-build.sh" "$@"; py_rc=$?
