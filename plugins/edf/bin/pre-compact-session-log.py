...
Output: appends a section to .edf/session-drafts/YYYY-MM-DD-<session8>.md — scratch,
self-ignored via .edf/.gitignore, one file per session (later compactions append).
Drafts older than DRAFT_RETENTION_DAYS are pruned on each run, so no skill has to own
their deletion. /feature-end reads the feature's drafts as cost-retrospective input.
"""
...
def draft_path_for(project_root: pathlib.Path, session_id: str) -> pathlib.Path:
    """Return this session's draft path under the self-ignored .edf/session-drafts/."""
    edf_dir = project_root / ".edf"
    drafts = edf_dir / "session-drafts"
    drafts.mkdir(parents=True, exist_ok=True)
    ignore = edf_dir / ".gitignore"
    if not ignore.exists():
        ignore.write_text("*\n", encoding="utf-8", newline="\n")
    sid8 = session_id[:8]
    existing = sorted(drafts.glob(f"*-{sid8}.md"))
    if existing:
        return existing[0]
    return drafts / f"{datetime.now().strftime('%Y-%m-%d')}-{sid8}.md"


def prune_old_drafts(drafts_dir: pathlib.Path, days: int = DRAFT_RETENTION_DAYS) -> None:
...
def write_draft(draft_path: pathlib.Path, section: str, session_id: str) -> None:
    if draft_path.exists():
        ...
        header = (
            ...
            f"_Auto-generated by the pre-compact hook. Scratch — never committed, "
            f"pruned after {DRAFT_RETENTION_DAYS} days._\n\n"
