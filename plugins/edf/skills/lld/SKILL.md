...
**Always read first (both modes):** `kb/architecture.md` — the curated catalogue of canonical helpers, types, and composition roots (API composition pattern section). The LLD must reference kb entries by import path in its "Reused helpers — DO NOT re-implement" table at the top of Part B; code samples elsewhere must call canonical helpers by name, never inline their bodies. If a topic in scope is not yet covered by the kb, flag it as an Open Question and propose the entry — do not silently invent a new helper.
