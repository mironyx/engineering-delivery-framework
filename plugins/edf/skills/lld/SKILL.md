...
- **External API surfaces verified against docs.** For every concrete claim about a *third-party* surface the LLD did not read from this repo — SDK/library config shapes, function or hook signatures, cloud-provider resource arguments, query-language or IaC semantics — did I verify it against the official documentation with `WebFetch`/`WebSearch` rather than recalling it from memory? Author and reviewer both reason from training knowledge, which can be confidently wrong in a way that stays internally consistent, so a second read does not catch it. For each verified surface, cite the doc URL and the library version next to the claim. If a surface genuinely cannot be verified (no accessible docs, or ambiguous), mark it inline as `> **Unverified — recall-based:**` and set its `Verified` column to `Unverified`, so the risk is visible at review and implementation rather than hidden. Scope this to surfaces the implementing `/feature` agent will code directly against — do not chase every transitive dependency.
- **External Surfaces table complete.** Does Part B open with an `## External Surfaces` table, and does it list *every* surface whose contract is defined outside this repo — packages and SDKs, but also protocol and wire-format specifications, cloud/IaC providers, and third-party HTTP APIs? A surface with no dependency-manifest entry (an MCP or OAuth spec revision, a webhook payload format) is the easiest to omit and the most dangerous to omit, because there is no manifest version for anyone downstream to fall back on. Every row needs a pinned version, constraint, or dated revision (`hashicorp/azurerm ~> 3.108`, `@azure/storage-blob@12.18.0`, `MCP 2025-06-18`) — not "Azure Storage SDK", and not blank. If a surface is genuinely version-agnostic, write `version-agnostic` and say why.
- **`New to repo` determined by grep, not memory.** For each row, did I actually grep the repo to decide whether this phase is the first use of that surface? This column gates doc-reading during implementation and research during PR review, so a wrong `No` silently disables both. When in doubt, mark `Yes` — the cost is one doc read, and it is paid once per surface for the life of the repo.
- **Prompt economy (verbatim LLM prompt text).** Does this LLD include verbatim text destined for an LLM system prompt (instructions, worked examples, field descriptions, output format rules)? If so, run a short prompt-economy pass before marking the section ready: (a) check for internally contradictory logical wording — e.g. "strictly greater than or equal to" mixes a strict and a non-strict comparator and will confuse both the LLM and the reviewer; (b) check for non-functional meta-commentary — dates, review-attribution text, rationale for why an example was rejected — this adds token cost on every generation call and provides zero signal to the LLM; (c) verify every referenced source and test file path exists in the repo. These defects are cheap to fix in the LLD but expensive to rework post-PR: each one doubles the pre-PR cost of the feature.
