...
- **External API surfaces verified against docs.** For every concrete claim about a *third-party* surface the LLD did not read from this repo — SDK/library config shapes, function or hook signatures, cloud-provider resource arguments, query-language or IaC semantics — did I verify it against the official documentation with `WebFetch`/`WebSearch` rather than recalling it from memory? Author and reviewer both reason from training knowledge, which can be confidently wrong in a way that stays internally consistent, so a second read does not catch it. Cite the doc URL next to the claim, and record the surface in the External Surfaces table. If a surface genuinely cannot be verified, mark it inline as `> **Unverified — recall-based:**` and set its `Verified` column to `Unverified`. Scope this to surfaces the implementing `/feature` agent codes directly against — do not chase every transitive dependency.
- **External Surfaces table complete.** Does Part B open with an `## External Surfaces` table listing *every* surface whose contract is defined outside this repo, each with a pinned version, constraint, or dated revision? Surfaces with no dependency-manifest entry (an MCP or OAuth spec revision, a webhook payload format) are the easiest to omit and the worst to omit — nothing downstream can recover a version for them.
- **`New to repo` decided by grep, not memory.** Did I actually grep for prior use of each surface? The column gates doc-reading during implementation and research at PR review, so a wrong `No` silently disables both. When unsure, mark `Yes` — the cost is one doc read, paid once per surface for the life of the repo.
