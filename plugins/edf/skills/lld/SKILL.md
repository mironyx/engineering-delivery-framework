...
- **Version pinning for every third-party surface.** Every third-party tool, SDK, cloud provider, Terraform provider, or external API referenced in this LLD must have an explicit version or version constraint in the spec (e.g. `hashicorp/azurerm ~> 3.108`, `@azure/storage-blob@12.18.0`, `OpenAI API 2024-11`). Not "Azure Storage SDK" — pin the version so the design is the anchor for drift tracking. If the surface is genuinely version-agnostic (e.g. a generic HTTP client), state that explicitly rather than omitting.
