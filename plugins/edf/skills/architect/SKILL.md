...
- **API route internal decomposition and reused helpers are `/lld`'s responsibility.** The `/lld` skill's template and self-critique pass already enforce API route internal decomposition, kernel-based reused helpers tables, and no-raw-queries rules. `/architect` delegates to `/lld` — do not duplicate these rules here.
